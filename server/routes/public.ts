...
import { bad, bool, isEmail, notFound, rateLimit, str } from '../lib/http.ts';
import { buildFeed } from '../lib/gcal.ts';
import { sessionContext, when } from '../lib/notify.ts';
import { invoiceDetail, paymentInstructions, periodLabel } from '../lib/invoices.ts';
...
type FamilyRow = Pick<StudentRow, 'id' | 'name' | 'parent_name' | 'portal_token'>;
type BookingRow = SessionRow & { student_name: string | null; portal_token: string | null };

export default function publicRoutes(db: DB, notify: Notifier) {
  const r = Router();
  const one = <T>(sql: string, ...a: SQLInputValue[]) => q1<T>(db, sql, ...a);
  const origin = (req: Request): string => `${req.protocol}://${req.get('host')}`;

  const familyByToken = (token: unknown): FamilyRow | null => {
    const st = token ? one<FamilyRow>('SELECT id, name, parent_name, portal_token FROM students WHERE portal_token = ? AND active = 1', String(token)) : null;
    if (token && !st) throw notFound('This family link is no longer valid. Ask for a new one.');
    return st ?? null;
  };

  const openChange = (sessionId: number) => one<Pick<ChangeRow, 'kind' | 'new_start_at' | 'new_end_at' | 'reason' | 'requested_at'>>(
  ...
  const lastDecision = (sessionId: number) => one<Pick<ChangeRow, 'kind' | 'new_start_at' | 'status' | 'tutor_note' | 'decided_at'>>(
  ...
  const insideNotice = (s: Pick<SessionRow, 'start_at'>): boolean => isLateCancel(db, s.start_at) === 1;
  ...
  r.get('/report/:token', (req, res) => {
    const rep = one<Pick<ReportRow, 'body' | 'period_from' | 'period_to' | 'sent_at'> & { student: string; portal_token: string }>(
  ...
  r.post('/waitlist', rateLimit({ max: 5, windowMs: 60 * 60 * 1000 }), (req, res) => {
    ...
    const days = (Array.isArray(b.weekdays) ? b.weekdays as unknown[] : []).map(Number).filter((d: number) => Number.isInteger(d) && d >= 0 && d <= 6);
    const weekdays = [...new Set(days)].sort().join(',');
    const existing = one<WaitlistRow>("SELECT * FROM waitlist WHERE email = ? AND student_name = ? COLLATE NOCASE AND status = 'active'", email, student);
    let token;
    if (existing) {
      run(db, 'UPDATE waitlist SET parent_name = ?, phone = ?, weekdays = ?, note = ? WHERE id = ?',
        parent, str(b.phone, 40), weekdays, str(b.note, 1000), existing.id);
      token = existing.token;
    } else {
      token = newToken();
      run(db, 'INSERT INTO waitlist (parent_name, student_name, email, phone, weekdays, note, token) VALUES (?, ?, ?, ?, ?, ?, ?)',
        parent, student, email, str(b.phone, 40), weekdays, str(b.note, 1000), token);
      const n = scalar<CountRow>(db, "SELECT COUNT(*) AS n FROM waitlist WHERE status = 'active'").n;
  ...
  r.post('/waitlist/:token/leave', (req, res) => {
    const info = run(db, "UPDATE waitlist SET status = 'removed' WHERE token = ? AND status = 'active'", String(req.params.token));
  ...
  r.post('/requests', rateLimit({ max: 8, windowMs: 60 * 60 * 1000 }), (req, res) => {
    ...
    const rate = family ? scalar<{ rate_cents: number }>(db, 'SELECT rate_cents FROM students WHERE id = ?', family.id).rate_cents : getSettings(db).default_rate_cents;
    db.prepare(
    ...
    const s = sessionContext(db, scalar<{ id: number }>(db, 'SELECT id FROM sessions WHERE token = ?', token).id);
    if (s.family_email) {
      run(db, "UPDATE waitlist SET status = 'booked' WHERE email = ? COLLATE NOCASE AND status = 'active'", s.family_email);
    }
  ...
  const byToken = (token: unknown): BookingRow => {
    const s = one<BookingRow>('SELECT s.*, st.name AS student_name, st.portal_token FROM sessions s LEFT JOIN students st ON st.id = s.student_id WHERE s.token = ?', String(token));
  ...
  r.post('/booking/:token/change', limiter, (req, res) => {
    ...
    if (kind === 'reschedule') {
      const ns = str(req.body.new_start_at);
      if (!isDateTime(ns)) throw bad('Pick a new time');
      const ne = addMinutes(ns, minutesBetween(s.start_at, s.end_at));
      const day = ns.slice(0, 10);
      const busy = occupied(db, day, addDays(day, 1), null, { excludeSessionId: s.id });
      const lenOk = openSlots(db, day, addDays(day, 1), busy).some((x) => x.start_at === ns);
      const clash = busy.some((b) => b.start_at < ne && ns < b.end_at);
  ...
  r.post('/booking/:token/change/withdraw', limiter, (req, res) => {
    const s = byToken(req.params.token);
    const info = run(db, "UPDATE change_requests SET status='withdrawn', decided_at=? WHERE session_id=? AND status='pending'",
      nowLocal(), s.id);
  ...
  r.get('/family/:token', (req, res) => {
    ...
    materializeRecurring(db);
    const rows = all<SessionRow & { student_name: string }>(db,
      `SELECT s.*, ? AS student_name FROM sessions s
        WHERE s.student_id = ? AND s.start_at >= ? AND s.start_at < ? AND s.status <> 'declined'
        ORDER BY s.start_at`,
      fam.name, fam.id, `${addDays(today(), -28)}T00:00`, `${addDays(today(), 56)}T00:00`);
    const now = nowLocal();
    res.json({
      student: fam.name,
      ...
      reports: all<Pick<ReportRow, 'token' | 'period_from' | 'period_to' | 'sent_at'>>(db, "SELECT token, period_from, period_to, sent_at FROM reports WHERE student_id = ? AND status = 'sent' ORDER BY sent_at DESC LIMIT 12", fam.id),
      invoices: all<Pick<InvoiceRow, 'token' | 'period' | 'amount_cents' | 'status'>>(db, "SELECT token, period, amount_cents, status FROM invoices WHERE student_id = ? AND status <> 'void' ORDER BY period DESC LIMIT 6",
