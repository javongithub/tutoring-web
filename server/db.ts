...
function migrate(db: DB): void {
  for (const [table, column, ddl] of MIGRATIONS) {
    const cols = all<{ name: string }>(db, `PRAGMA table_info(${table})`).map((c) => c.name);
    if (!cols.includes(column)) db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${ddl}`);
  }
}

export function openDb(file: string = process.env.DB_PATH || 'data/tutoring.db'): DB {
...
// Typed query helpers. node:sqlite returns loosely typed rows; the caller states the row shape.
export const one = <T>(db: DB, sql: string, ...args: Param[]): T | undefined => db.prepare(sql).get(...args) as T | undefined;
export const all = <T>(db: DB, sql: string, ...args: Param[]): T[] => db.prepare(sql).all(...args) as T[];
export const run = (db: DB, sql: string, ...args: Param[]) => db.prepare(sql).run(...args);
/** Like `one`, for queries that always return a row (COUNT, SUM, ...). */
export const scalar = <T>(db: DB, sql: string, ...args: Param[]): T => db.prepare(sql).get(...args) as T;

export function getSettings(db: DB): Settings {
  const out: Record<string, string | number> = { ...DEFAULT_SETTINGS };
  for (const { key, value } of all<{ key: string; value: string }>(db, 'SELECT key, value FROM settings')) {
    if (!(key in DEFAULT_SETTINGS)) continue;
    out[key] = typeof DEFAULT_SETTINGS[key as SettingKey] === 'number' ? Number(value) : value;
  }
  return out as Settings;
}

export function setSetting(db: DB, key: SettingKey, value: string | number): void {
  run(db, 'INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', key, String(value));
}
