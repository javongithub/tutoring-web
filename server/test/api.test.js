...
import { addDays, today, weekday, weekStart } from '../lib/time.js';
import { expandIcs, parseTutoringTitle } from '../lib/ics.js';
...
before(async () => {
  ...
  const app = createApp(db, { auth: makeAuth({ password: 'pw', secret: 's', secure: false }) });
...
test('public week hides names and details', async () => {
  ...
  const labels = body.occupied.filter((o) => o.start_at.startsWith(day)).map((o) => o.label).sort();
  assert.deepEqual(labels, ['Busy', 'Other student']);
  const starts = body.slots.filter((s) => s.start_at.startsWith(day)).map((s) => s.start_at.slice(11));
...
test('family portal shows own sessions by name, others anonymised', async () => {
  ...
  const onDay = body.occupied.filter((o) => o.start_at.startsWith(day));
  assert.deepEqual(onDay.map((o) => o.label), ['Alex', 'Other student', 'Busy']);
...
test('family cancel needs tutor approval', async () => {
  ...
  const row = body.rows.find((x) => x.cancel_reason === 'Sick');
  assert.equal(row.cancelled_by, 'client');
  assert.equal(row.late_cancel, 0);
  assert.equal(body.by_student.find((x) => x.name === 'Kid').client_cancels, 1);
  const week = (await call('GET', `/api/public/week?start=${day}`, null, { auth: false })).body;
  assert.ok(week.slots.some((s) => s.start_at === `${day}T19:00`), 'cancelled slot opens back up');
});
...
test('family move request holds the new slot; approval moves one week of a weekly schedule', async () => {
  ...
  const session = fam.upcoming.find((s) => s.start_at === `${day}T15:00`);
  ...
  assert.ok(!week.slots.some((s) => s.start_at === `${day}T19:00`), 'requested slot is held');
  assert.ok(week.occupied.some((o) => o.start_at === `${day}T15:00`), 'original still booked until approval');
  ...
  assert.ok(week.slots.some((s) => s.start_at === `${day}T19:00`), 'declined hold released');
  ...
  assert.deepEqual(body.sessions.map((s) => s.start_at), [`${day}T19:00`], 'moved, and weekly rule did not regenerate 15:00');
});
...
test('iCal feed: weekly recurrence, exdate, override, tutoring detection', () => {
  ...
  const alexTimes = ev.filter((e) => e.uid === 'a').map((e) => e.start_at);
  assert.deepEqual(alexTimes, [`${mon}T17:00`, `${addDays(mon, 2)}T15:30`, `${addDays(mon, 9)}T15:30`]);
  const fusion = ev.find((e) => e.uid === 'b');
...
test('Google Calendar push: create, update on move, delete on cancel; no echo on pull', async () => {
  const { createGcal, pushDirty, APP_MARKER } = await import('../lib/gcal.js');
  ...
  const gcal = createGcal({ account: { email: 'sa@x.iam', key: privateKey.export({ type: 'pkcs8', format: 'pem' }) }, fetchImpl: fake });
  ...
  const feed = await call('GET', `/api/public/feed/${db.prepare("SELECT value FROM settings WHERE key='feed_token'").get().value}.ics`, null, { auth: false });
