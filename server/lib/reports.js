...
export function aiAvailable(env = process.env) {
  return !!env.ANTHROPIC_API_KEY;
}

// Session notes -> the user message. Only first name, subject/grade and notes are sent.
export function buildPrompt(student, sessions, from, to) {
...
export function createReporter({ client } = {}) {
  let api = client;
  const getClient = () => {
  ...
  return {
    async draft(student, sessions, from, to) {
      ...
      return { body: text, truncated: response.stop_reason === 'max_tokens', usage: response.usage };
    },
  };
}
