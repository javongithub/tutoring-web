...
import { type DB, all, getSettings, run, setSetting } from '../db.ts';
import type { SessionRow } from '../types.ts';
import { addDays, TZ, today } from './time.ts';

export interface ServiceAccount { email: string; key: string }
/** Response from the Calendar API: an event (has `id`) or `{ gone: true }` for 404/410. */
type ApiResult = { id?: string; gone?: boolean };
export interface Gcal {
  api(method: 'POST' | 'PATCH' | 'DELETE', path: string, body?: unknown): Promise<ApiResult>;
  email: string;
}
/** A session joined with the student fields the calendar event shows. */
export interface EventSource extends SessionRow {
...
export function createGcal({ account, fetchImpl = fetch }: { account: ServiceAccount; fetchImpl?: typeof fetch }): Gcal {
  ...
  async function api(method: 'POST' | 'PATCH' | 'DELETE', path: string, body?: unknown): Promise<ApiResult> {
    ...
    return res.status === 204 ? {} : res.json() as Promise<ApiResult>;
  }

  return { api, email: account.email };
}

const KEEP: SessionRow['status'][] = ['pending', 'confirmed', 'completed', 'no_show'];

export function eventBody(s: EventSource) {
...
export async function pushDirty(db: DB, gcal: Gcal | null): Promise<{ pushed: number }> {
  const { gcal_calendar_id: cal } = getSettings(db);
  if (!gcal || !cal) return { pushed: 0 };
  const rows = all<EventSource>(db,
    ...
    `${addDays(today(), -7)}T00:00`, `${addDays(today(), 70)}T00:00`);
  ...
  try {
    for (const s of rows) {
      if (!KEEP.includes(s.status)) {
        ...
        if (r.gone) run(db, 'UPDATE sessions SET gcal_dirty = 1 WHERE id = ?', s.id); // recreate next pass
      } else {
        const r = await gcal.api('POST', calPath, eventBody(s));
        done.run(r.id ?? null, s.id);
      }
      pushed++;
    }
    setSetting(db, 'gcal_error', '');
    if (pushed) setSetting(db, 'gcal_synced_at', new Date().toISOString());
  } catch (e) {
    setSetting(db, 'gcal_error', e instanceof Error ? e.message : String(e));
    throw e;
  }
  return { pushed };
}
...
export function buildFeed(db: DB): string {
  const rows = all<EventSource>(db,
    `SELECT s.*, st.name AS student_name, st.parent_name, st.subject, st.next_plan AS student_next_plan
       FROM sessions s LEFT JOIN students st ON st.id = s.student_id
      WHERE s.status IN ('pending','confirmed','completed','no_show') AND s.start_at >= ? AND s.start_at < ?`,
    `${addDays(today(), -30)}T00:00`, `${addDays(today(), 70)}T00:00`);
  const esc = (v: string): string => String(v).replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/([,;])/g, '\\$1');
  const dt = (s: string): string => s.replace(/[-:]/g, '') + '00';
