...
import { type DateStr, type DateTimeStr, TZ, fromMs } from './time.ts';

interface RRule { freq: string; interval: number; byday: number[] | null; until: string | null; count: number | null }
interface VEvent {
  uid?: string;
  summary?: string;
  status?: string;
  ...
  rrule?: RRule;
  recurrenceId?: DateTimeStr | null;
  exdates: (DateTimeStr | null)[];
}
type Params = Record<string, string>;
export interface IcsInstance { uid: string; summary: string; start_at: DateTimeStr; end_at: DateTimeStr; from_app: boolean }
...
function parseLine(line: string): { name: string; params: Params; value: string } | null {
  const idx = line.indexOf(':');
  if (idx < 0) return null;
  const [name, ...paramParts] = line.slice(0, idx).split(';');
  const params: Params = Object.fromEntries(paramParts.map((p) => p.split('=')));
...
function toLocal(value: string, params: Params): DateTimeStr | null {
...
function parseRRule(v: string): RRule {
  const r: Params = Object.fromEntries(v.split(';').map((kv) => kv.split('=')));
  return {
    freq: r.FREQ,
...
const toMsLocal = (s: string): number => Date.parse(`${s}:00Z`);

type TimedEvent = VEvent & { uid: string; start: DateTimeStr; end: DateTimeStr };
const isTimed = (e: VEvent): e is TimedEvent => !!(e.start && e.end && !e.allDay);

// Expand events into concrete instances overlapping [from, to) (local date strings).
export function expandIcs(text: string, from: DateStr, to: DateStr): IcsInstance[] {
  const events = parseIcs(text).map((e) => ({ ...e, uid: e.uid ?? '' })).filter(isTimed);
  const overrides = new Map<string, Map<string, TimedEvent>>(); // uid -> (recurrenceId -> event)
  ...
  const push = (e: TimedEvent, start: DateTimeStr, end: DateTimeStr) => {
  ...
  for (const e of events.filter((x) => !x.recurrenceId)) {
    ...
    const ov = overrides.get(e.uid) ?? new Map<string, TimedEvent>();
    const emit = (start: DateTimeStr) => {
      if (e.exdates.includes(start)) return;
      const o = ov.get(start);
      if (o) { push(o, o.start, o.end); return; }
...
export function parseTutoringTitle(summary: string): { name: string; parent: string } | null {
...
export const isTutoring = (summary: string | null | undefined): boolean => /tutor/i.test(summary || '');
