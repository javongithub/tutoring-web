...
export function createNotifier(db, { smtp = smtpConfig(), origin = () => process.env.PUBLIC_URL || '' } = {}) {
  const ins = db.prepare('INSERT INTO outbox (channel, to_addr, subject, body, link) VALUES (?, ?, ?, ?, ?)');
  const base = (reqOrigin) => (getSettings(db).public_url || origin() || reqOrigin || '').replace(/\/$/, '');
...
export function sessionContext(db, sessionId) {
  return db.prepare(
...
export const when = (s) => fmtWhen(s.start_at, s.end_at);

// Send whatever is queued. `sendMail({from,to,subject,text})` and `fetchImpl` are injectable for tests.
export async function flushOutbox(db, { smtp = smtpConfig(), sendMail, fetchImpl = fetch } = {}) {
  ...
  const rows = db.prepare('SELECT * FROM outbox WHERE sent_at IS NULL AND attempts < ? ORDER BY id LIMIT 50').all(MAX_ATTEMPTS);
  ...
  for (const m of rows) {
    try {
      ...
      fail.run(String(e.message || e).slice(0, 500), m.id);
    }
  }
...
async function sendPush(m, fetchImpl) {
...
export function queueReminders(db, notifier) {
  const st = getSettings(db);
  ...
  ).all(`${tomorrow}T00:00`, `${addDays(tomorrow, 1)}T00:00`);
  const mark = db.prepare('UPDATE sessions SET reminded = 1 WHERE id = ?');
