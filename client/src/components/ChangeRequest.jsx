...
import PublicWeek from './PublicWeek.jsx';
import { ErrorText, Modal, useAction } from './ui.jsx';
...
export default function ChangeRequest({ session, family, onClose, onDone }) {
  ...
  const submit = () => run(async () => {
    await post(`/public/booking/${session.token}/change`, { kind, reason, new_start_at: slot?.start_at });
    onDone();
  });
  ...
  return (
    <Modal title="Change this session" onClose={onClose} wide={kind === 'reschedule'}>
      ...
      <div className="actions">
        ...
        <button className="btn primary" disabled={busy || (kind === 'reschedule' && !slot)} onClick={submit}>
